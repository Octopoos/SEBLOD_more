...
require_once JPATH_SITE.'/plugins/cck_storage_location/joomla_viewlevel/joomla_viewlevel.php';

// Class
class plgCCK_Storage_LocationJoomla_Viewlevel_Integration extends plgCCK_Storage_LocationJoomla_Viewlevel
{
	...
	public static function onCCK_Storage_LocationAfterDispatch( &$data, $uri = array() )
	{
		...
		if ( !$uri['layout'] ) {
			...
			$data['options']->set( 'add_alt_link', 'index.php?option=com_users&view=level&layout=edit&cck=1' );
			if ( $do == 1 ) {
	...
	public static function onCCK_Storage_LocationAfterRender( &$buffer, &$data, $uri = array() )
	{
		...
		$data['return_view']	=	'levels';
		$data['search']			=	'#<a href="(.*)index.php\?option=com_users&amp;task=level.edit&amp;id=([0-9]*)"#';
	}
}
?>