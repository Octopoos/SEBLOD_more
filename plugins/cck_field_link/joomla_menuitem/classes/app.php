...
// Class
class plgCCK_Field_LinkJoomla_Menuitem_App extends plgCCK_FieldGeneric_App
{	
	// -------- -------- -------- -------- -------- -------- -------- -------- // Export
	...
	protected static function _updateImportItemId( $field, &$field_join, $data )
	{
		...
		if ( $itemId ) {
			$itemId	=	JCckDatabase::loadResult( 'SELECT id FROM #__menu WHERE path LIKE "%'.(string)$itemId.'"' );
